...
_KERNEL_SPEC=importlib.util.spec_from_file_location('quality_loop_operator_kernel',ROOT/'scripts'/'kernel.py')
...
def main():
    ...
    malformed=kernel.evaluate_case(None)
    if malformed.get('status')!='INVALID': failures.append({'id':'non-dict-case-guard','got':malformed})
