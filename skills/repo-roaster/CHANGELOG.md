...
- Added ten production-surface packs including API contracts, infrastructure-as-code, frontend state, multi-tenant, auth, billing, migration, realtime, async jobs and AI-agent/MCP.
...
- Challenger -> Defender -> Arbiter review protocol for high-impact engineering findings.
- Source manifests, system model, critical-invariant/surface/state-transition/failure-domain ledgers.
- CLI, desktop, mobile, AI-agent-system, supply-chain, and build/release review coverage.
- Evidence-strength, scope-sensitivity, categorical materiality, review outcomes, and quality gates.
- Richer repository inventory: package managers, lockfiles, CI, infra, auth/jobs/data surfaces, languages, largest files, and git diff surface.
...
- Deterministic inventory comparison and stronger recheck closure semantics with explicit verification status/change basis.

### Changed

- Upgraded the machine report contract and validator to `cometweb.repo-roaster/v5`.
- Tightened CRITICAL admission, absence-proof discipline, DIFF coverage, and insufficient-evidence stop conditions.

## [1.3.0] - 2026-09-21

### Added

- Repository review profiles, critical-surface ledger, blast-radius classes, containment state, deployment model, and DIFF change-surface contract.
- RELIABILITY and OPERABILITY lenses plus structured verification-gap discipline.
...
- RECHECK mode with stable finding keys and a resolution ledger.
- Critical-invariant ledger and CRITICAL linkage to declared invariants or critical paths.
- Runtime, log, and metric evidence states alongside source/config/test/history evidence.
- Defect classes, fix-scope classification, structured verification contracts, and root-cause grouping.
- Expanded repository inventory for infrastructure, observability, data surfaces, risk surfaces, workspaces, source roots, and optional git snapshot.
- Review dimensions for state machines, API compatibility, background jobs, authorization ownership, infrastructure coupling, and recheck regressions.

### Changed

- Upgraded machine-readable report schema to v3.
- Tightened CRITICAL reachability and verification rules.
- Expanded false-positive and contradiction checks around guards, wrappers, tests, flags, and runtime evidence.
